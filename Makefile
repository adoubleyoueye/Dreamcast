...
dev:
	${bin_path}/python3 manage.py runserver 8030
...
run:
	...
	${bin_path}/gunicorn project.wsgi:application --bind localhost:8030

shell:
	@echo 'To activate the venv use source ~/.venv/dreamcast_api/bin/activate . Use deactivate to exit'

lint:
	# starts a pipenv shell, shows autopep8 diff and then fixes the files
	# does the same for isort
	@echo '---Running autopep8---'
	${bin_path}/autopep8 dreamcast_api -r -d
	${bin_path}/autopep8 dreamcast_api -r -i
	@echo '---Running isort---'
	${bin_path}/isort dreamcast_api --diff
	${bin_path}/isort dreamcast_api --atomic
